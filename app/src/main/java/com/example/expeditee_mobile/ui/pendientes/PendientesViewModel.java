...
public class PendientesViewModel extends AndroidViewModel {
    ...
    public void obtenerClientesPendientes() {
        ...
        obtenerPropietario(new OnObtenerPropietarioListener() {
            @Override
            public void onObtenerPropietarioSuccess(int idUsuario) {
                ...
                Call<ArrayList<Usuario>> call = end.obtenerMisPendientes(token, idUsuario);
                        ...
                        if (response.isSuccessful()) {
                            ...
                            if (clientesPendientes != null) {
                                ...
                                for (Usuario usuario : clientesPendientes) {
                                    obtenerImagenUsuario(usuario.getId(), usuario, new OnImagenCargadaListener() {
                                        @Override
                                        public void onImagenCargada() {
                                            ...
                                            if (imagenesCargadas == totalUsuarios) {
                                                pendientesMutable.setValue(clientesPendientes);
                                            }
                                        }
                                    });
                                }
    ...
    public void obtenerImagenUsuario(int id, Usuario usuario, OnImagenCargadaListener listener) {
        SharedPreferences sp = context.getSharedPreferences("token.xml", Context.MODE_PRIVATE);
        String token = sp.getString("token", "");
    ...
    public void obtenerPropietario(final OnObtenerPropietarioListener listener) {
        SharedPreferences sp = context.getSharedPreferences("token.xml", Context.MODE_PRIVATE);
        String token = sp.getString("token", "");
